...
?>
	...
	<div class="col-md-4">
        <?php
    if (checkacl('servers', true)) {
?>
	    <h3 class="text-center">Servers</h3>
        <p class="lead text-center"><?php
        echo count($backupservers);
?></p>
        <?php
    }
?>
...
?>
	...
	<div class="col-md-8 col-md-offset-2">
        ...
        <?php
    if (checkacl('disk', true)) {
?>
        <h4>Disk <?php
        echo round((disk_total_space($config['path']) - disk_free_space($config['path'])) / disk_total_space($config['path']) * 100, 2);
?>%</h4>
        <div class="progress">
            <div class="progress-bar" role="progressbar" aria-valuenow="60" aria-valuemin="0" aria-valuemax="100" style="width: <?php
        echo round((disk_total_space($config['path']) - disk_free_space($config['path'])) / disk_total_space($config['path']) * 100, 2);
?>%;">
            </div>
        </div>
